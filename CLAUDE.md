...
### บอทใน LINE เรียก `models.sh` ไม่ได้ แต่ถามเป็นภาษาคนได้

`scripts/` **ไม่ได้ถูก mount** เข้าคอนเทนเนอร์ (มีแค่ `data/` กับ `workspace/`)
แต่ในคอนเทนเนอร์มี `git` `curl` `python3` และ `LITELLM_KEY` ครบ และยิง
`http://llm-litellm:4000/model/info` ได้ HTTP 200

**ถามบอทตรง ๆ ได้เลย ไม่ต้องแก้อะไร** — ทดสอบแล้ว 2026-09-03 มันดึงแค็ตตาล็อก
มานับได้ 122 ตัวและกรอง `th/*` ให้ถูกต้อง พร้อมบอกเองว่ากรองด้วยชื่อเพราะไม่เจอ
tag ที่ขอ (ไม่ได้แกล้งทำเป็นว่าเจอ)

ตัดสินใจไว้ว่า **ไม่ mount `scripts/` เพิ่ม** — สคริปต์ตอบได้แค่หมวดที่เขียนไว้
ส่วนบอทตอบคำถามที่ยังไม่มีใครคิดถึงได้
...
### skill ที่บอทเขียนเอง — เราไม่ยุ่ง

บอทบันทึก skill ของตัวเองไว้ที่ `data/skills/` ในคอนเทนเนอร์ (2026-09-03 มี 8 ตัว
ที่ไม่ใช่ bundled) **เจ้าของคือบอท** เราไม่เอาเข้า `skills-keep.txt` และไม่ seed ให้

⚠️ `trim-skills.sh` เคยลบ skill พวกนี้ทิ้งหมดโดยไม่มีใครรู้ — ขั้นตอน 2b กวาดทุกอย่าง
ที่ไม่อยู่ใน keep-list ด้วย `rm -rf` และ skill ที่เขียนเอง**ใส่ keep-list ไม่ได้**
เพราะขั้นตอน 1 ตรวจว่าต้องมีใน bundled ก่อน แก้แล้วให้ข้ามตัวที่ไม่มีใน
`/opt/hermes/skills` — ยืนยันด้วยการรันจริง `skills: 12 → 12`

สำเนาไว้อ่านอยู่ที่ `docs/skills/` — **ตกยุคได้ ของจริงคือในคอนเทนเนอร์เสมอ**
