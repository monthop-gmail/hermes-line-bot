...
## รอบสี่ (2026-08-27) — วัดใหม่ด้วยโจทย์ที่ถูก

พบว่า `probe-thai.sh` ใช้ **โจทย์แถวบน** เป็นค่าเริ่มต้นมาตลอด ซึ่งเป็นโจทย์ที่
**ให้ `oc/gpt-oss-120b` ผ่าน 3/3 ทั้งที่มันหลุด** — แปลว่าผล "ไทย 3/3" ทุกตัวที่
เคยรายงานไปวัดด้วยเครื่องมือที่จับสิ่งที่กำลังหาไม่ได้

เปลี่ยนโจทย์ยากเป็นค่าเริ่มต้นแล้ว และวัดตัวที่อยู่ใน production ใหม่:

| model | บทบาท | โจทย์ยาก |
|---|---|---|
| `mi/ministral-14b` | main | **ไทย 3/3** |
| `zen/hy3` | fallback 1 | **ไทย 3/3** |

ทั้งคู่ผ่านจริง ไม่ต้องเปลี่ยน chain — แต่ถ้าไม่ได้ไล่ดูก็จะไม่มีวันรู้ว่า
ผลเดิมเชื่อไม่ได้
